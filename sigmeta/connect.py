"""Native cognis-connect emit for sigmeta — forward findings to any platform.

Maps sigmeta's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    sigmeta ... --format json | sigmeta-emit --to stix
    sigmeta-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out.pop('label', None)
        out.pop('freq_hz', None)
        out.pop('band', None)
        out.pop('modulation', None)
        out.pop('bandwidth_hz', None)
        out.pop('service_hint', None)
        out['title'] = rec.get('label', '')
        out['severity'] = 'medium'
        out['type'] = 'signal'
        out['description'] = 'Signal metadata parsed and classified'
        out['tags'] = ['signal', 'metadata']
